...
namespace CSF
{
    ...
    public sealed class Command : IConditionalComponent, IParameterContainer
    {
        ...
        public Command(Module module, MethodInfo method, string[] aliases)
        {
            ...
            var remainderParameters = Parameters.Where(x => x.Flags.HasFlag(ParameterType.Remainder));
            if (remainderParameters.Any())
            {
                ...
                if (!Parameters.Last().Flags.HasFlag(ParameterType.Remainder))
            ...
            where T : IContext
        {
            ...
            return ParseResult.Success(((ParseResult)result).Result, -1);
        }
            ...
            where T : IContext
        {
            ...
            {
                return ExecuteResult.Error(ex.Message, ex);
            }
        }
