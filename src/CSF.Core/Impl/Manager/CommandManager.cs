...
namespace CSF
{
    ...
    public class CommandManager
    {
            ...
            where T : IContext
        {
            ...
            var (command, args) = ((MatchManyResult)checkResult).Result.First();
        ...
        protected virtual SearchResult OnCommandNotFound(IContext context)
            => SearchResult.Error($"Failed to find command with name: {context.Name}.");
        ...
        /// <param name="cancellationToken">Token to be used to signal the code to break.</param>
        /// <returns>An asynchronous <see cref="ValueTask"/> holding the result of the executed command.</returns>
        public virtual ValueTask<MatchResult> MatchAsync<T>(T context, IEnumerable<Command> commands, CancellationToken cancellationToken)
            where T : IContext
        {
                ...
                foreach (var command in commands)
                {
                    ...
                    if (command.MaxLength <= contextLength)
                        foreach (var parameter in command.Parameters)
                            if (parameter.Flags.HasFlag(ParameterType.Remainder))
                                yield return command;
            ...
            if (!matches.Any())
            {
                ...
                return MatchResult.Success(new[] { overload });
            }

            return MatchResult.Success(matches);
        }
        ...
        /// <param name="context">The context containing information necessary to search and match the command.</param>
        /// <returns>The result of the executed command.</returns>
        protected virtual MatchResult OnBestOverloadUnavailable(IContext context)
            => MatchResult.Error($"Failed to find overload that best matches input: {context.Name}.");
            ...
            where T : IContext
        {
            ...
            if (!matches.Any())
                return MatchResult.Error(failureResult.ErrorMessage, failureResult.Exception);

            return MatchManyResult.Success(matches);
        }
    }
}
